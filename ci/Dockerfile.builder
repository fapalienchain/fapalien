...
# fapalien_hash
RUN git clone https://github.com/fapalienchain/fapalien_hash
RUN cd fapalien_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} fapalien
RUN useradd -u ${USER_ID} -g fapalien -s /bin/bash -m -d /fapalien fapalien
...
RUN mkdir /fapalien-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /fapalien-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /fapalien-src
