Cross-compiliation of Fapalien Core
===============================

Fapalien Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Fapalien.
...
When building Fapalien Core, use
...
Note that for WSL the Fapalien Core source path MUST be somewhere in the default mount file system, for
example /usr/src/fapalien, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the Fapalien Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Fapalien Core source code).
