...
- [Development guidelines](#development-guidelines)
    - [General Fapalien Core](#general-fapalien-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Fapalien Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/fapalienchain/fapalien/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/fapaliend -printtoconsole
```
...
Fapalien Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Fapalien Core, where everyone uses
...
pay attention to for reviewers of Fapalien Core code.

General Fapalien Core
----------------------
...
- Avoid using locale dependent functions if possible. You can use the provided
  ...
  - These functions are known to be locale dependent:
    ...
    `fputws`, `fapaliennf`, `fwprintf`, `getdate`, `getwc`, `getwchar`, `isalnum`,
    ...
    `vdprintf`, `versionsort`, `vfprintf`, `vfapaliennf`, `vfwprintf`, `vprintf`,
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Fapalien Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Fapalien Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `fapalien-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `fapalien-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Fapalien Core, the wallet was always
