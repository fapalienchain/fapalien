...
*Setup instructions for a Gitian build of Fapalien Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Fapalien
...
- [Building Fapalien Core](#building-fapalien-core)
...
Clone the git repositories for Fapalien Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/fapalienchain/fapalien
...
in the Fapalien Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Fapalien Core
----------------

To build Fapalien Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Fapalien Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fapalien/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/fapalienchain/fapalien
...
For example:
```bash
URL=https://github.com/crowning-/fapalien.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit fapalien=${COMMIT} --url fapalien=${URL} ../fapalien/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fapalien=${COMMIT} --url fapalien=${URL} ../fapalien/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fapalien=${COMMIT} --url fapalien=${URL} ../fapalien/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the fapalien git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../fapalien/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/fapalien
SIGPATH=/some/root/path/fapalien-detached-sigs

./bin/gbuild --url fapalien=${BTCPATH},signature=${SIGPATH} ../fapalien/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fapalien-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fapalien-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/fapalien-osx-build.assert
```
...
[fapalien/gitian.sigs](https://github.com/fapalienchain/gitian.sigs/) repository, or if that's not possible to create a pull
request.
