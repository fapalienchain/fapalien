...
const char * const BITCOIN_CONF_FILENAME = "fapalien.conf";
const char * const BITCOIN_PID_FILENAME = "fapaliend.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "fapaliend -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FapalienCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FapalienCore
    // Mac: ~/Library/Application Support/FapalienCore
    // Unix: ~/.fapaliencore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FapalienCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FapalienCore";
#else
    // Unix
    return pathRet / ".fapaliencore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty fapalien.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Fapalien Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2023, nEndYear) + "The Fapalien Core developers";
    }
