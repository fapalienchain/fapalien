...
PACKAGE_VERSION=$($SRC_PATH/src/fapaliend --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/fapaliend $DEB_BIN
cp $SRC_PATH/src/fapalien-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/fapalien-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/fapaliend.1 $DEB_MAN
cp $SRC_DOC/man/fapalien-cli.1 $DEB_MAN
cp $SRC_DOC/man/fapalien-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/fapaliend.bash-completion $DEB_CMP/fapaliend
cp $SRC_PATH/contrib/fapalien-cli.bash-completion $DEB_CMP/fapalien-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/fapaliend.1
gzip --best -n $DEB_MAN/fapalien-cli.1
gzip --best -n $DEB_MAN/fapalien-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/fapaliend $DEB_BIN/fapalien-cli
