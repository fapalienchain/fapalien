...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/FapalienParams"
else
    PARAMS_DIR="$HOME/.fapalien-params"
fi
...
DOWNLOAD_URL="https://github.com/fapalienchain/fapalien/releases/tag/1.2.15.3"
...
IPFAPMD="$(command -v ipfs || echo '')"
CURLCMD="$(command -v curl || echo '')"

# fetch methods can be disabled with FAP_DISABLE_SOMETHING=1
FAP_DISABLE_WGET="${FAP_DISABLE_WGET:-}"
#FAP_DISABLE_IPFS="${FAP_DISABLE_IPFS:-}"
FAP_DISABLE_CURL="${FAP_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$FAP_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFAPMD" ] || ! [ -z "$FAP_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$FAP_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Fapalien parameters!
...
This script will fetch the Fapalien SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Fapalien SNARK parameters. Note that it is
